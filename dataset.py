...
def open_zip(zip_path_or_url):
    if isinstance(zip_path_or_url, str) and zip_path_or_url.startswith("http"):
        try:
        ...
        # Если zip_path_or_url — это локальный путь
        try:
            return zipfile.ZipFile(zip_path_or_url, 'r')
        except Exception as e:
            print(f"Error opening local zip {zip_path_or_url}: {e}")
            return None
...
def process_archive(url):
    """
    Обрабатывает архив по URL (используется при онлайн-загрузке, если нужно)
    """
    try:
        r = requests.get(url)
        r.raise_for_status()
    ...
    try:
        ...
        return []
    return process_zipfile(zf)

def process_zipfile(zf):
    records = []
    current_ob = None
